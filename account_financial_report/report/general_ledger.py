...
class GeneralLedgerReportMoveLine(models.TransientModel):
    ...
    journal = fields.Char()
    account = fields.Char()
    conta_credito = fields.Char()
    conta_debito = fields.Char()
...
SELECT
    ...
    j.code AS journal,
    a.code AS account,
    (select aa.code_reduced from account_move_line am, account_account aa
	  where aa.id = am.account_id and am.move_id = m.id and am.debit > 0 limit 1) as conta_debito,
    (select aa.code_reduced from account_move_line am, account_account aa
	  where aa.id = am.account_id and am.move_id = m.id and am.credit > 0 limit 1) as conta_credito,
    CASE
        WHEN
    ...
    '""" + _('No partner allocated') + """' AS partner,
            """
        query_inject_move_line += """
    CASE WHEN ml.invoice_id is not null
        THEN
            (SELECT nfe_number_static FROM account_invoice WHERE 
            id = ml.invoice_id)::varchar
        WHEN
            ml.move_id is not null
        THEN
            CONCAT_WS(' / ', ARRAY(SELECT nfe_number_static FROM account_invoice WHERE 
            id in (select invoice_id from account_move_line 
            where full_reconcile_id in (select full_reconcile_id from account_move_line 
            where move_id = ml.move_id and full_reconcile_id is not null) 
            ))::varchar,  NULLIF(ml.ref, ''), NULLIF(ml.name, ''))
        ELSE
            CONCAT_WS(' - ', NULLIF(ml.ref, ''), NULLIF(ml.name, ''))
    END AS label,
