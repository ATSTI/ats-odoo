...
class AccountMoveReport(models.Model):
    ...
    fatura = fields.Char(string='Fatura/NF')
    ...
    company_id = fields.Many2one('res.company', 'Empresa', default=lambda self: self.env['res.company']._company_default_get('account.move.report'))
    dias = fields.Integer(string='Dias')
    account_id = fields.Many2one('account.account', string='Conta Contábil')
    move_id = fields.Many2one('account.move', string='Movimento')
        ...
        self._cr.execute("""
            CREATE OR REPLACE VIEW account_move_report AS (
                SELECT 
                    ...
                    || '(' || rp.cnpj_cpf || ')' as fornecedor
                    ,(CASE WHEN ai.nfe_number_static > 0 THEN CAST(ai.nfe_number_static as VARCHAR) ELSE ai.reference END) as fatura
