...
class ImportXmlWizard(models.TransientModel):
    ...
    def retorna_data(self, ide):
        ...
        if hour_emissao[0] == '00:00:00':
            ...
            datehour_emissao = day_emissao[0] + ' ' + hour_emissao[0][:8]
            dt = datetime.strptime(datehour_emissao, '%Y-%m-%d %H:%M:%S')
            dt = dt + timedelta(hours=+3)
        datetime_obj_emissao = dt # datetime.strptime(datehour_emissao, '%Y-%m-%d %H:%M:%S')
    ...
    def get_invoice_eletronic_vals(self, nfe, fatura, edoc):
        ...
        if fatura.product_document_id.code == '55' or self._context.get('tipo') == '55':
            try:
                if nfe.getchildren()[0].infNFe:
                    ide = nfe.getchildren()[0].infNFe.ide
                    dest = nfe.getchildren()[0].infNFe.dest
                    emit = nfe.getchildren()[0].infNFe.emit
                    protNFe = nfe.getchildren()[1].infProt
                else:
                    ide = nfe.getchildren()[0].ide
                    dest = nfe.getchildren()[0].dest
                    emit = nfe.getchildren()[0].emit
                    protNFe = nfe.getchildren()[1].infProt
        ...
        if emit.CNPJ:
            partner_doc = str(partner_doc).zfill(14)
        ...
        dt_final = datetime.strftime(self.data_entrada, '%Y-%m-%d')
        data_entrada = dt_final + ' ' + '15:00:00'
        vals = {
        ...
        if finalidade not in ('1','2','3','4'):
            vals['finalidade_emissao'] = '1'
        vals['tp_emiss_cte'] = finalidade
        if self._context.get('tipo') == '57' or \
            self._context.get('tipo') == '67' or \
            fatura.product_document_id.code == '57' or \
            fatura.product_document_id.code == '67':
    ...
    def create_order_line(self, item, nfe, edoc, num_item):
        ...
        if not uom_id:
            uom_id = self.env['uom.uom'].search([
                ('active','=',False),
                ('name', 'ilike', str(item.prod.uCom))], limit=1)
            uom_id.write({'active': True})
            uom_id = uom_id.id
            if not uom_id:
                uom_id = self.env['uom.uom'].sudo().create({
                    'type_uom':'ext',
                    'name':str(item.prod.uCom),
                    'category_id':4,
                    'uom_type': 'bigger',
                }).id,
        product = ''
        ...
        #edoc_val = edoc.eletronic_item_ids.search(['|',
        #     ('valor_bruto', '>', valor_bruto - 0.01), 
        #     ('valor_bruto', '<', valor_bruto + 0.01)])
        edc = edoc.eletronic_item_ids
        for item_edoc in edoc.eletronic_item_ids:
            if item_edoc.product_id.id == product.id and \
               item_edoc.valor_bruto > (valor_bruto - 0.01) and \
               item_edoc.valor_bruto < (valor_bruto + 0.01) and \
               not item_edoc.importado:
        ...
        if codigo_diferentes:
            for item_edoc in edoc.eletronic_item_ids:
                if item_edoc.product_id.id == product.id:
                    if item_edoc.valor_bruto > (valor_bruto - 0.01) and \
                        item_edoc.valor_bruto < (valor_bruto + 0.01):                  
                        edc = item_edoc
                        item_num = int(item.get('nItem'))
                        # 16/12/2019
                        #item_edoc.num_item = item_num
                        item_edoc.num_item = num_item
        else:
            edc = edoc.eletronic_item_ids
        
        for item_edoc in edc:
            #if product.id == 1037 and item_edoc.product_id.id == 1037:
            if item_edoc.num_item != num_item and num_item == item_num:
                item_edoc.num_item = num_item

            if item_edoc.product_id.id == product.id and \
                valor_bruto > (item_edoc.valor_bruto - 0.009) and \
                valor_bruto < (item_edoc.valor_bruto + 0.009) and \
                not item_edoc.importado:
                ...
                """
            #elif valor_bruto > (item_edoc.valor_bruto - 0.009) and \
            #    valor_bruto < (item_edoc.valor_bruto + 0.009) and \
            #    num_item != item_edoc.num_item:
            #        item_edoc.num_item = num_item
            item_num += 1
        return True    
    ...
    def get_itens(self,nfe, edoc):
        ...
        for det in nfe.NFe.infNFe.det:
            if cod_iguais == det.prod.cProd:
                self.localiza_produto = False
                break
            cod_iguais = det.prod.cProd
        """
        total_itens = 0
        for det in nfe.NFe.infNFe.det:
            item = self.create_order_line(det, nfe, edoc, int(det.get('nItem')))
            total_itens += 1
        # rotina pra conferir se o num_item esta correto
        #import pudb;pu.db
        total_item_edoc = 0
        for item_edoc in edoc.eletronic_item_ids:
            total_item_edoc += 1
        for det in nfe.NFe.infNFe.det:
            # para cada item da nota existe um item na fatura
            if total_item_edoc == total_itens:
                for item_edoc in edoc.eletronic_item_ids:
                    if det.prod.xProd == item_edoc.name or \
                        item_edoc.valor_bruto > (det.prod.vProd -0.01) and \
                        item_edoc.valor_bruto < (det.prod.vProd + 0.01):
                            # 16/12/2019
                            #item_edoc.num_item = int(det.get('nItem'))
                            item_edoc.name = det.prod.xProd
                            if det.prod.uCom != item_edoc.uom_id.name: 
                                uom_id = self.env['uom.uom'].search([
                                ('name', 'ilike', str(det.prod.uCom))], limit=1).id
                                if uom_id:
                                    item_edoc.uom_id = uom_id
                #valor_bruto = det.prod.vProd
                #for item_edoc in edoc.eletronic_item_ids:
        return True
    ...
    def consulta_produtos_nfe(self):
        ...
        items = []
        if not fatura.product_document_id.code:
            raise UserError('o Documento Produtos na Fatura não foi informado.')
        if fatura.product_document_id.code == '55' or self._context.get('tipo') == '55':
            if nfe.getchildren()[0].infNFe:
                nfe_ids = nfe.getchildren()[0].infNFe.det
            else:
                nfe_ids = nfe.getchildren()[0].det
        if fatura.product_document_id.code == '57' or self._context.get('tipo') == '57':
            nfe_ids = nfe.CTe.infCte.vPrest
            self.consulta_nfe()
            return
        if self._context.get('tipo') == '67' or fatura.product_document_id.code == '67':
            nfe_ids = nfe.CTeOS.infCte.vPrest
            self.consulta_nfe()
            return
        for det in nfe_ids:
