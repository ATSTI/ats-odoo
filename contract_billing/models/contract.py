...
class ContractContract(models.Model):
    ...
    def _prepare_recurring_invoices_values(self, date_ref=False):
        ...
        result = super()._prepare_recurring_invoices_values(date_ref)

        for contract, invoice_vals in zip(self, result):
            invoice_date = invoice_vals.get("invoice_date") or fields.Date.today()

            mes_ano = invoice_date.strftime("%Y-%m")
            contrato = contract.code or str(contract.id)

            invoice_vals.update({
                "ref": f"{contrato}-{mes_ano}",
