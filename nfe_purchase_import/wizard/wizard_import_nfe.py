...
#---------------------------------------------------------------------------------------------------------------------#
    ...
    def busca_produto_cadastro(self, item, nfe):
        ...
        if not product:
            emit = nfe.NFe.infNFe.emit
            partner_doc = emit.CNPJ if hasattr(emit, 'CNPJ') else emit.CPF
            partner_id = self.env['res.partner'].search([('cnpj_cpf_stripped', '=', str(partner_doc))]).id
            #import pudb;pu.db
    ...
    def create_order_line(self, item, nfe, order_id, nitem, estoque):
        ...
        order_line = self.env['purchase.order.line'].create({
            ...
            'product_qty': quantidade, 'price_unit': preco_unitario, 'uom_id':product.uom_id.id,
            'order_id':order_id, 'quantity': quantidade, 'fiscal_price': preco_unitario, 'fiscal_quantity': quantidade,
        })
        return order_line
