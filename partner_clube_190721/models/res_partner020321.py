...
class ResPartner(models.Model):
    ...
    def insere_cartao(self, partner_id, cartao, id_user=0):
        ...
        if id_user == 0:
            ...
            if cli_fb[0][0]:
                id_user = cli_fb[0][0]
        # CARTAO 
        sqld = 'select id from Cards where idUser = %s' %(id_user)
    ...
    def altera_socio(self, partner_id, nome):
        ...
        for cli in cli_fb:
            ...
            if partner_id.cartao:
                ...
                idSqlite = 'SELECT id FROM Cards where idUser = %s' %(str(cli[0]))
                id_result = con.query(idSqlite)
                if not id_result[0][0]:
    ...
    def action_atualiza_clientes(self):  
        ...
        hj = datetime.now()
        hj = hj - timedelta(days=1200) # AQUI ESTOU PEGANDO A DATA DE HOJE MENOS 5 DIAS 
        hj = datetime.strftime(hj,'%Y-%m-%d %H:%M:%S')
