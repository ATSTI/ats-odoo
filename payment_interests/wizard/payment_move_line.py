...
class AccountPayment(models.Model):
    ...
    def default_get(self, fields):
        if 'move_line_id' in fields:
            ...
            if not move_line_id:
                # e por FATURA
                inv = self.env['account.invoice'].browse(rec['invoice_ids'][0][2])
                empresa = inv.company_id.id
                if inv:
                    move_line_ids = inv.move_id.line_ids
                    # for linha in inv.move_id.line_ids:
                        # if linha.account_id.user_type_id.type == 'receivable':
                        #     move_line_id = linha.id
                if not move_line_ids:
                    return super(AccountPayment, self).default_get(fields)
            else:
                move_line_ids = self.env['account.move.line'].browse(move_line_id)
                empresa = move_line_ids[0].company_id.id
            multa = 0.0
            juros = 0.0
            valor_original = 0.0
            for move_line in move_line_ids:
                if move_line.account_id.user_type_id.type == 'receivable' and \
                    not move_line.reconciled:
                    current_date = date.today()
                    cpn = self.env['res.company.interest'].browse(empresa)
                    quantidade_dias = abs((current_date - move_line.date_maturity).days)
                    valor_original += move_line.amount_residual
                    if move_line.date_maturity >= current_date or quantidade_dias < cpn.tolerance_interval:
                        multa += 0.0
                        juros += 0.0
                    else:
                        quantidade_dias = quantidade_dias - cpn.tolerance_interval
                        multa += move_line.amount_residual * (cpn.multa/100)
                        juros += cpn.company_id.currency_id.round(
                            (cpn.rate/100) * quantidade_dias * move_line.amount_residual)
            rec.update({
                'multa': multa,
                'juros': juros,
    ...
    def _onchange_incluir_juros(self):
        if self.incluir_juros:
            ...
            self.payment_difference_handing = 'reconcile'
