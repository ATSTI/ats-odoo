odoo.define("l10n_br_pos_nfce.nfe-xml", function (require) {
    ...
    class NFeXML {
        ...
        mountDestinataryTag() {
            ...
            // 👇 pega do botão do PDV
            const cpfFromButton = (this.order.get_extra_note && this.order.get_extra_note()) || '';
            const cleanedCPF = cpfFromButton.replace(/\D/g, '');

            // se tem CPF digitado → usa ele
            ...
            // sem CPF → consumidor não identificado
            if (!client || client.is_anonymous_consumer) {
                return;
            }
            ...
            if (!clientCnpjCpf) return;

            const cleaned = clientCnpjCpf.replace(/\D/g, '');
            const isCPF = cleaned.length === 11;

            return {
                [isCPF ? "CPF" : "CNPJ"]: cleaned,
                xNome: client.name || '',
                indIEDest: "9",
            };
        }
