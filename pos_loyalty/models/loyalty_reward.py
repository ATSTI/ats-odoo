...
class LoyaltyReward(models.Model):
    ...
    name = fields.Char(string='Prêmio', size=32, index=True,
                       required=True)
    type = fields.Selection(selection=[('gift', 'Presente'),
                                       ('discount', 'Desconto'),
                                       ('resale', 'Revenda')],
                            string='Tipo', required=True,
                            help='Type of the reward')
    minimum_points = fields.Float(string='Mínimo de pontos',
                                  help='Minimum amount of points the customer'
                                       ' must have to qualify for this reward')
    point_cost = fields.Float(string='Valor dos Pontos',
                              help='Cost of the reward per monetary unit '
                                   'discounted')
    discount = fields.Float(help='The discount percentage')
    discount_max = fields.Float(string='Limite de Desconto',
