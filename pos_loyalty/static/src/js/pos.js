    ...
    models.Order = models.Order.extend({
        ...
        get_won_points: function(){
            ...
            for (var i = 0; i < orderLines.length; i++) {
                ...
                for (var j = 0; j < rules.length; j++) {
                    ...
                    for (var k=0; k < promocao_item.length; k++) {
                       if (a[k] != product.id) {
                           promo_add = 1;
                       }    
                    }
                    ...
                    if (promo_add == 1) {
                        self.promocao = rule.pp_product;
                        self.promocao_item.push(product.id);
                    }
                    promo_add = 0;
        ...
        get_new_points: function() {
            ...
            if (!this.pos.loyalty) {
                ...
                /*if (this.get_won_points() == 99) {*/
                if (self.promocao == 99) {
                    if (self.promocao_item != 0) {
        ...
        get_available_rewards: function(){
            ...
            if (!client) {
                ...
                for (var i = 0; i< this.pos.loyalty.rewards.length; i++) {
                    ...
                    if (reward.minimum_points > this.get_won_points()) {
                        continue;
                    } else if(reward.type === 'gift'  &&
                          reward.point_cost > this.get_won_points()) {
                        continue;
                    } else if(reward.type === 'discount' &&
                            (discount_reward_set || reward.point_cost > this.get_won_points())){
                        continue;
                    }
                    rewards.push(reward);
                    }
        ...
        apply_reward: function(reward){
            ...
            if (reward.type === 'gift') {
                ...
                for (var i=0;i<this.get_orderlines().length;i++) {
                    if (this.get_orderlines()[i].product.id == product.id) {
                        var discount = round_pr(this.get_orderlines()[i].quantity * this.get_orderlines()[i].price * reward.discount,crounding)
                    }
                }
        ...
        finalize: function(){
            ...
            var client = this.get_client();
            var rew = this.pos.loyalty.rewards;
            if ( client ) {
                client.loyalty_points = this.get_new_total_points();
                //for (var i=0;i<rew.length;i++) {
	            //    if (this.pos.get_client().loyalty_points >= rew[i].point_cost) {
	            //        alert('O cliente pode clamar a recompensa: ' + rew[i].name + ' que custa ' + rew[i].point_cost + ' pontos pois possui: ' + this.pos.get_client().loyalty_points + ' pontos.')} };
                this.pos.gui.screen_instances.clientlist.partner_cache.clear_node(client.id);
            }
            //else {
            //    for (var i=0;i<rew.length;i++) {
	        //        if (this.get_won_points() >= rew[i].point_cost) {
	        //            alert('O cliente pode clamar a recompensa: ' + rew[i].name + ' que custa ' + rew[i].point_cost + ' pontos pois possui: ' + this.get_won_points() + ' pontos.')} };
            //}
            _order_super.finalize.apply(this, arguments);
        },
