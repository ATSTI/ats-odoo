...
class ProductTemplate(models.Model):
    ...
    def write(self, vals):
        ...
        if 'list_price' in vals:
            values['name'] = 'Preço Venda'
            values['ocorrido'] = 'Alterado'
            values['valor_anterior'] = self.list_price
            values['valor_novo'] = vals.get('list_price')
            self.sped_ids.create(values)                    
        if 'margin' in vals:
            values['name'] = 'Margen'
            values['ocorrido'] = 'Margen'
            values['valor_anterior'] = self.margin
            values['valor_novo'] = vals.get('margin')
            self.sped_ids.create(values)                    
