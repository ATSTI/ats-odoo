...
class SpedFile(models.Model):
    ...
    def registro0000(self, inv):
        ...
        arq._registro_abertura.DT_INI = dta_s # datetime.strptime(dta_s, '%Y-%m-%d') # self.transforma_data(g_intervalo[0])
        arq._registro_abertura.DT_FIN = dta_e # datetime.strptime(dta_e, '%Y-%m-%d') # self.transforma_data(g_intervalo[1])
        ...
        if self.company_id.accountant_id:
            ...
            #else:
            #    contador = self.company_id.accountant_id.legal_name
            #    cpf = self.company_id.accountant_id.cnpj_cpf    
    ...
    def query_registro0205(self, item):
        ...
        for alterado in resposta_produto:
            ...
            if not alterado.valor_anterior:
                continue
    ...
    def query_registroC100(self, fatura):
        ...
        for resposta_nfe in nfe_ids:    
            if (resposta.product_document_id or resposta.state in ['open','paid','cancel']) and \
                ...
                if resposta_nfe.emissao_doc == '1' and not resposta_nfe.state == 'cancel' and resposta_nfe.chave_nfe[6:20] != self.limpa_formatacao(resposta_nfe.partner_id.company_id.cnpj_cpf):
                    #import pudb;pu.db
